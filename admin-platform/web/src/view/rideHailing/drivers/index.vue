<template>
  <div class="ride-drivers">
    <RidePageHeader title="司机管理" subtitle="司机档案、状态、认证与运营信息">
    ...
    <div class="gva-table-box">
      ...
      <el-table v-loading="loading" :data="tableData" row-key="id" style="width: 100%">
        <el-table-column label="司机编号" prop="personNo" min-width="150" />
    ...
    <el-drawer v-model="detailVisible" title="司机详情" size="480px">
      <el-descriptions v-if="detail" :column="1" border>
        <el-descriptions-item label="司机编号">{{ detail.personNo }}</el-descriptions-item>
...
import { listDriverRecords, getDriverRecord, batchDriverStatus } from '@/api/rideHailing/drivers'
import { exportPersons } from '@/api/rideHailing/workorder05'
import RidePageHeader from '@/components/RidePageHeader/index.vue'

defineOptions({ name: 'RideHailingDrivers' })
...
const getTableData = async () => {
  loading.value = true
  try {
    const res = await listDriverRecords({ page: page.value, pageSize: pageSize.value, ...searchForm })
    if (res.code === 0) {
...
const openDetail = async (row) => {
  const res = await getDriverRecord(row.id)
  if (res.code === 0) { detail.value = res.data; detailVisible.value = true }
}

const toggleStatus = (row, status) => {
  ElMessageBox.confirm(`确认${status === 'enabled' ? '启用' : '禁用'}司机 ${row.name}？`, '提示', { type: 'warning' }).then(async () => {
    const res = await batchDriverStatus({ ids: [row.id], status })
