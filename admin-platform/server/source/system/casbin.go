...
var carpoolTripReviewCasbinRules = []adapter.CasbinRule{
	{Ptype: "p", V0: "888", V1: "/carpool/trip/:id/review", V2: "POST"},
	{Ptype: "p", V0: "8881", V1: "/carpool/trip/:id/review", V2: "POST"},
	{Ptype: "p", V0: "9528", V1: "/carpool/trip/:id/review", V2: "POST"},
}
...
func (i *initCasbin) DataInserted(ctx context.Context) bool {
	...
	if !ensureCarpoolTripReviewCasbinRules(db) {
		return false
	}
...
func ensureCarpoolTripReviewCasbinRules(db *gorm.DB) bool {
	for _, rule := range carpoolTripReviewCasbinRules {
		var existing adapter.CasbinRule
		if err := db.Where(adapter.CasbinRule{
			Ptype: rule.Ptype,
			V0:    rule.V0,
			V1:    rule.V1,
			V2:    rule.V2,
