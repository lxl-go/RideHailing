...
import (
	...
	autoReq "ride-hailing/admin-server/plugin/ai/model/request"
	autoRes "ride-hailing/admin-server/plugin/ai/model/response"
)
...
func nodeLabel(n autoModel.CliScenarioNode) string {
	...
	label := "`" + strings.TrimSpace(n.CommandName) + "`"
	if alias := strings.TrimSpace(n.Alias); alias != "" {
		label += "（别名：" + alias + "）"
	}
	return label
