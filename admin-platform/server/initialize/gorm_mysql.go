...
import (
	...
	"ride-hailing/admin-server/initialize/internal"
	"time"
...
func initMysqlDatabase(m config.Mysql) *gorm.DB {
	...
	mysqlConfig := mysqlDialectorConfig(m)
	// 数据库配置
	general := m.GeneralDB
...
func mysqlDialectorConfig(m config.Mysql) mysql.Config {
	return mysql.Config{
		DSN:                       m.Dsn(), // DSN data source name
		DefaultStringSize:         191,     // string 类型字段的默认长度
		SkipInitializeWithVersion: false,   // 根据版本自动配置
		DisableDatetimePrecision:  true,    // 兼容 MySQL 8 的 CURRENT_TIMESTAMP 默认值
	}
}
