...
import (
	...
	"ride-hailing/admin-server/utils/ast"
)

func configureAutoCodeTestRoot(t *testing.T) {
	t.Helper()
	...
	global.GVA_CONFIG.AutoCode = config.Autocode{
		Root:   serverRoot,
		Server: "",
		Web:    filepath.Join("..", "web", "src"),
	}
	t.Cleanup(func() { global.GVA_CONFIG.AutoCode = original })
}
