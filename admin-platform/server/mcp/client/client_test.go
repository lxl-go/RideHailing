...
func requireMCPIntegration(t *testing.T) {
	t.Helper()
	if os.Getenv("GVA_MCP_CLIENT_INTEGRATION") != "1" {
		t.Skip("set GVA_MCP_CLIENT_INTEGRATION=1 and start http://localhost:8888/sse to run MCP client integration tests")
	}
}
