  ...
  <view class="page">
    ...
    <scroll-view class="message-list" scroll-y :scroll-into-view="lastMessageId">
      <view v-if="messages.length === 0" class="empty-chat">
        ...
        <text class="empty-tip">可直接发送消息和司机沟通行程细节</text>
      </view>
      <view
...
const readAccessToken = () => {
  try {
    return String(uni.getStorageSync('passengerAccessToken') || '')
  } catch {
    return ''
  }
}
...
const connect = () => {
  ...
  userId.value = userId.value || String(uni.getStorageSync('passengerUserId') || '')
  const accessToken = readAccessToken()
  const query = `role=${role}&user_id=${encodeURIComponent(userId.value)}&access_token=${encodeURIComponent(accessToken)}`
  const url = buildSocketUrl(`/api/v1/passenger/orders/${encodeURIComponent(orderId.value)}/chat/ws?${query}`)
...
const sendMessage = () => {
  ...
  const clientMsgId = `p-${orderId.value}-${Date.now()}`
  const payload = {
    type: 'chat',
    content,
...
onLoad((query) => {
  ...
  userId.value = String(query?.userId || query?.user_id || uni.getStorageSync('passengerUserId') || '')
  connect()
})
