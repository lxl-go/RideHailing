...
function createTraceId() {
  return `passenger-${Date.now()}-${Math.random().toString(16).slice(2)}`
}
...
function saveSession(session = {}) {
  ...
  if (userId) uni.setStorageSync('passengerUserId', userId)
  if (accessToken) uni.setStorageSync('passengerAccessToken', accessToken)
  if (refreshToken) uni.setStorageSync('passengerRefreshToken', refreshToken)
}
...
function buildHeaders(extraHeaders = {}, traceId = '') {
  ...
  const token = readStorage('passengerAccessToken')
...
function normalizeErrorMessage(message, statusCode) {
  ...
  if (lower.includes('sms send failed') || lower.includes('ihuyi sms') || lower.includes('internal error')) return '服务暂时不可用，请稍后再试'
  if (lower.includes('order not found')) return '订单不存在或已被处理，请刷新后重试'
  if (raw) return raw
  if (statusCode === 401) return '登录已过期，请重新登录'
  if (statusCode === 404) return '接口不存在，请确认网关服务已启动'
...
function doRequest(config, retried) {
  ...
  return new Promise((resolve) => {
    uni.request({
      ...
      success: (res) => {
        const data = res.data ?? {}
        console.info('[passenger-api]', traceId, config.method || 'GET', config.url, res.statusCode, data?.code ?? '')
        ...
        if (res.statusCode >= 400) {
          ...
          console.warn('[passenger-api:error]', traceId, config.method || 'GET', config.url, res.statusCode, msg)
        ...
        console.warn('[passenger-api:business]', traceId, config.method || 'GET', config.url, data?.code, msg)
        if (!config.silent) uni.showToast({ title: msg, icon: 'none' })
        resolve({ ...data, msg })
      },
      fail: () => {
        console.warn('[passenger-api:network]', traceId, config.method || 'GET', config.url)
...
function refreshToken() {
  const refreshTokenValue = readStorage('passengerRefreshToken')
